...
class CIFAR100CNN(nn.Module):
    def __init__(self, use_batchnorm=True, dropout_rate=0.5, activation='ReLU', init_type='he'):
        ...
        self.pool = nn.MaxPool2d(2, 2)
        self.dropout_layer = nn.Dropout(dropout_rate)
        self.fc1 = nn.Linear(128 * 4 * 4, 256)  # CIFAR-100: 32x32 -> after 3 pools: 4x4
        self.fc2 = nn.Linear(256, 10)  # Output layer for 100 classes
...
def get_data_loaders(dataset_name, batch_size, data_augmentation=False, label_noise_ratio=0.0, data_subset_ratio=1.0, cifar100_classes_seed=None):
    """获取指定数据集的数据加载器"""
    # 数据增强变换
    ...
    # --- 新增：CIFAR-100 数据加载逻辑 ---
        ...
        # 1. 加载完整的CIFAR-100训练集
        full_cifar100_train = torchvision.datasets.CIFAR100(
            root='./data', train=True, download=True, transform=train_transform
        )
        ...
        # 2. 使用给定的种子选择10个类
        original_classes = list(range(100)) # CIFAR-100有100个类
        if cifar100_classes_seed is not None:
            original_classes_state = random.getstate()
            random.seed(cifar100_classes_seed)
            selected_classes = sorted(random.sample(original_classes, 10))
            random.setstate(original_classes_state) # 恢复随机状态
        else:
             # 如果没有提供种子，则默认选前10个类以保证确定性行为
            selected_classes = original_classes[:10]

        print(f"[DEBUG] CIFAR-100: Selected classes for this run: {selected_classes}")

        # 3. 创建掩码或索引列表，找出属于这10个类的样本
        train_indices = []
        for idx, (_, target) in enumerate(full_cifar100_train):
            if target in selected_classes:
                train_indices.append(idx)
        
        test_indices = []
        for idx, (_, target) in enumerate(test_cifar100):
            if target in selected_classes:
                test_indices.append(idx)

        # 4. 创建映射：原始类ID -> 新的连续类ID (0-9)
        class_mapping = {old_label: new_label for new_label, old_label in enumerate(selected_classes)}
        print(f"[DEBUG] CIFAR-100: Class mapping: {class_mapping}")

        # 5. 创建子集和映射类别的数据集
        # 这里需要一个自定义的Dataset类来应用类标签映射
        class MappedSubset(Dataset):
            def __init__(self, subset_dataset, indices, class_mapping):
                self.subset = Subset(subset_dataset, indices)
        ...
        full_train_dataset = MappedSubset(full_cifar100_train, train_indices, class_mapping)
        test_dataset = MappedSubset(test_cifar100, test_indices, class_mapping)
    ...
    if dataset_name == 'stl10':
        ...
        val_size = max(int(len(full_train_dataset) * 0.01), 100) # 取 1% 或至少 100 张
        #val_size = min(500, len(full_train_dataset)) # CIFAR-100每个类只有500个样本，总共10个类
        val_indices = indices[:val_size]
        train_indices = indices[val_size:]
    else:
        ...
        for i in train_indices:
            if random.random() < label_noise_ratio:
                if dataset_name in ['stl10', 'svhn']:
                    # Assuming labels attribute exists for svhn and stl10
                    # This might need adjustment based on actual dataset structure
                    # For SVHN, targets is usually 'labels'
                    # For STL10, it's usually 'labels'
                    # Let's assume the attribute name is consistent after loading
                    # The original code used 'targets' for CIFAR, MNIST, FashionMNIST
                    # and 'labels' for SVHN, STL10.
                    # We'll keep the logic assuming it works.
                    # If issues arise, we might need to check the specific dataset object.
                    if hasattr(full_train_dataset, 'labels'):
                        full_train_dataset.labels[i] = random.randint(0, 9)
                    elif hasattr(full_train_dataset, 'targets'):
                         # This handles cases where the attribute might be 'targets' even in SVHN/STL10 after Subset wrapping
                         # or if the condition was wrong initially.
                         # The safer way is to check the actual dataset object.
                         # However, for CIFAR100, which is similar to CIFAR10, it will be 'targets'.
                         pass # CIFAR100 will be handled below
                else: # Handles CIFAR10, MNIST, FashionMNIST, and CIFAR100 (which behaves like CIFAR10)
                    if hasattr(full_train_dataset, 'targets'):
                        full_train_dataset.targets[i] = random.randint(0, 9) # For CIFAR100, this will map to the remapped classes 0-9
                    elif hasattr(full_train_dataset, 'labels'):
                        full_train_dataset.labels[i] = random.randint(0, 9) # Fallback
    ...
    for i in tqdm(range(start_idx, end_idx), desc=f"{dataset_name} {sub_type} Task-{task_index}"):
        ...
        if 'snapshot' in sub_type:
            ...
            model = model_class(
                ...
                activation=chosen_activation_name, # 注意这里传递的是字符串名，因为初始化时需要
                init_type=chosen_init_type.lower() # 确保init_type是小写，与初始化函数一致
            )

            # 获取数据加载器
            # --- 为CIFAR100添加类选择种子 ---
            cifar100_seed = config_details.get('cifar100_classes_seed', 42) # 默认种子
            train_loader, val_loader, test_loader = get_data_loaders(
                dataset_name, batch_size=chosen_batch_size,
                data_augmentation=chosen_da, cifar100_classes_seed=cifar100_seed
            )

            # 设置优化器
            if chosen_optimizer == 'Adam':
                optimizer = optim.Adam(
                    model.parameters(), lr=chosen_lr, weight_decay=chosen_wd
                )
            elif chosen_optimizer == 'SGD':
                optimizer = optim.SGD(
                    model.parameters(), lr=chosen_lr, weight_decay=chosen_wd, momentum=0.9, nesterov=True
                )
            criterion = nn.CrossEntropyLoss()

            snapshot_epochs = [7, 8, 9]
            ...
            for epoch in snapshot_epochs:
                ...
                try:
                    ...
                    temp_model = model_class(
                        ...
                        init_type=chosen_init_type.lower()
                    )
        ...
        #低质量训练快照
            ...
            # 获取数据加载器
            # --- 为CIFAR100添加类选择种子 ---
            cifar100_seed = config_details.get('cifar100_classes_seed', 42)
            train_loader, val_loader, test_loader = get_data_loaders(
                dataset_name, batch_size=chosen_batch_size,
                data_augmentation=chosen_da, cifar100_classes_seed=cifar100_seed
            )
        ...
        #低质量数据问题
            ...
            train_loader, val_loader, test_loader = get_data_loaders(
                dataset_name, batch_size=chosen_batch_size,
                data_augmentation=chosen_da, label_noise_ratio=label_noise, cifar100_classes_seed=cifar100_seed
            )
    ...
    datasets_info = [
        # --- CIFAR-100 配置 ---
        {
            ...
            'high_quality_config': {
                ...
                'epochs': 20,
                'weight_decay': 1e-4,
                'cifar100_classes_seed': 9844, # 设定抽取10个类的随机种子
            },
            'low_quality_config': {
                'start_seed': 1001,
                #'learning_rate': 0.01,
                ...
                'epochs': 20,
                'weight_decay': 1e-4,
                'cifar100_classes_seed': 9844, # 低质量任务也使用相同的类
            }
        }
        # --- 结束新增 ---
    ]
